...
// Endpoint para listar estudiantes (nombre completo, email y curso)
app.get('/api/estudiantes', (req, res) => {
  db.query(
    `SELECT u.nombre, u.apellido, u.email, u.curso 
     FROM usuarios u 
     WHERE u.rol_id=1 AND u.estado='Activo'`,
    (err, rows) => {
      if (err) return res.status(500).json({ error: err });
      // Devuelve array de objetos { nombre, apellido, email, curso }, sin nulls
      res.json(rows.map(r => ({
        nombre: r.nombre || '',
        apellido: r.apellido || '',
        email: r.email,
